...
| Command | What |
|---------|------|
| `python tests/test_compliance.py` | 139 SKILL.md compliance assertions (unittest, not `make_checker()`) |
...
| `.skills-index.json` | User-local skills index. Gitignored — generated by auto-skill-select system, not a committed repo artifact |
...
- **.gitignore**: Excludes `.skills`, `.code-review-cache/`, `__pycache__/`, `*.pyc`, `.DS_Store`, `*.egg-info/`, `.env`, `.secret`, `.credentials`, `ccr-baseline.json`, `.skills-index.json`.
