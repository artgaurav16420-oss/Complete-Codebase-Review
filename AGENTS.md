...
| Command | What |
|---------|------|
| `python tests/test_compliance.py` | 139 compliance assertions (unittest, not `make_checker()`) |
...
| `.skills-index.json` | User-local skills index. Auto-generated, gitignored. |
...
- **.gitignore**: Excludes `.skills`, `.code-review-cache/`, `__pycache__/`, `*.pyc`,
  `.DS_Store`, `*.egg-info/`, `.env`, `.secret`, `.credentials`,
  `ccr-baseline.json`, `.skills-index.json`.
