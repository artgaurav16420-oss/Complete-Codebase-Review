...
Check "Has version field" { $content -match '(?m)^version: ' }
...
Check "Has dynamic effort via env var" { $content -match 'effort: \$\{CODE_REVIEW_EFFORT:-max\}' }
Check "No hardcoded model pin" { $content -notmatch '(?m)^model: opus' }
...
Check "Has sample output section" { $content -match 'Sample Output' -or $content -match 'Example Report' }
Check "Has changelog section" { $content -match 'Changelog' -or $content -match 'Change Log' }
...
Check "Has post-fix verification phase" { $content -match 'Phase 4g' -or $content -match 'Verify Fixes' }
...
Check "Covers single-pass bias failure" { $content -match '75%' -or $content -match 'halt if fewer' -or ($content -match 'parallel' -and $content -match 'complete') }
...
Check "Has at least 10 rules" { ($lines | Where-Object { $_ -match '^\|\s*\d+\s*\|' }).Count -ge 10 }
Check "Rule 1: Dynamic timeout per agent" { $content -match '\$\{CODE_REVIEW_TIMEOUT_SEC:-900\}' -and $content -match '75%' }
...
Check "Stops on insufficient agent coverage" { $content -match '75%' -or $content -match 'insufficient coverage' }
